      ...
      <div className="projects-container md:px-[3rem] px-[2rem] md:py-[5rem] py-[2rem] overflow-x-hidden">
        ...
        <div className="projects-grid flex flex-col md:flex-row items-start gap-4 mt-10 md:mt-20">
          {projects.map((project) => (
            <div className="project-card flex flex-col justify-between gap-2 bg-primary/10 md:p-8 p-4 flex-1 h-fit md:h-[35rem]" key={project.id}>
              <header className="project-header flex flex-row items-center justify-between">
                ...
                />
                <span className="project-status bg-primary rounded-full text-white font-medium md:px-4 md:py-2 px-2 py-1 text-[10px] md:text-base">{project.status}</span>
              </header>

              <section className="project-content flex flex-col gap-2">
                <h2 className="text-primary text-lg md:text-2xl font-thin">{project.title}</h2>
                {/* <p 
                ...
                <p className="project-description text-text-color text-justify text-sm md:text-base">{project.description}</p>

                <div className="project-features py-2">
                  <h3 className="font-medium text-sm md:text-base">Key Features</h3>
                  <ul className="md:px-4 px-5">
                    {project.features.map((feature, index) => (
                      <li className="list-disc text-sm md:text-base" key={index}>{feature}</li>
                    ))}
                  </ul>
                </div>
